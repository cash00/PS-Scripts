...
Param($Path="C:\Temp\MyInventory.xml")
 
Write-Host "Creating computer list" -ForegroundColor Green
#process list of computers filtering out those offline
$computers = Get-Content C:\Temp\myservers.txt | Where { Test-WSMan $_ -ErrorAction SilentlyContinue}
...
$cs = Get-Ciminstance Win32_Computersystem -ComputerName $computers | Select PSComputername,TotalPhysicalMemory,HyperVisorPresent,NumberOfProcessors,NumberofLogicalProcessors
$cs.PSComputerName
...
$data = $os.Computername -eq $Computer
...
$data = $cs.pscomputername -eq $Computer
